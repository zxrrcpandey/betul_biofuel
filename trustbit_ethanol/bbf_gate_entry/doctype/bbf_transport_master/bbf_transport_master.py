...
def add_vehicle(transporter, vehicle_number, vehicle_type, capacity_kg=0):
	...
	if not frappe.db.exists("BBF Vehicle Master", vehicle_number):
		frappe.get_doc({
		...
		frappe.db.set_value("BBF Vehicle Master", vehicle_number, "transporter", transporter)

	# Add to transporter's vehicle table if not already there
	doc = frappe.get_doc("BBF Transport Master", transporter)
