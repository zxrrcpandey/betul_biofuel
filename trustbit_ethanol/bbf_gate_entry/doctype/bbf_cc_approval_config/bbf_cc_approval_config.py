...
class BBFCCApprovalConfig(Document):
	...
	def _validate_route_steps(self):
		...
		route_doc = frappe.get_doc("BBF MR Approval Route", self.mr_approval_route)
		...
		for user, perm_name in existing_users.items():
			if user not in config_users:
				...
				other_cc_count = frappe.db.sql("""
					...
					INNER JOIN `tabBBF CC Approval Config` ccc ON ccc.name = ccu.parent
					WHERE ccu.user = %s
						AND ccu.parenttype = 'BBF CC Approval Config'
						AND ccc.is_active = 1
						AND ccc.name != %s
...
def get_cc_config(cost_center):
	"""Get active BBF CC Approval Config for a cost center, or None."""
	if not cost_center:
		return None
	try:
		config = frappe.get_doc("BBF CC Approval Config", cost_center)
		if config.is_active:
			return config
...
def get_user_allowed_cost_centers(user=None):
	...
	config_count = frappe.db.count("BBF CC Approval Config", {"is_active": 1})
	if config_count == 0:
	...
	ccs = frappe.db.sql("""
		...
		FROM `tabBBF CC Approval User`
		WHERE user = %s
			AND can_create_mr = 1
			AND parenttype = 'BBF CC Approval Config'
			AND parent IN (
				SELECT name FROM `tabBBF CC Approval Config`
				WHERE is_active = 1
			)
	""", user, as_dict=True)
