...
class BBFMaterialInspection(Document):
	def validate(self):
...
def create_material_inspection(gate_entry_name):
	...
	ge = frappe.get_doc("BBF Gate Entry", gate_entry_name)
	...
	if frappe.db.exists("BBF Material Inspection", {"gate_entry": ge.name}):
		return
	...
	# Create inspection
	insp = frappe.new_doc("BBF Material Inspection")
...
def check_inspection_sla():
	...
	for insp in pending:
		...
		if current_stage < 4 and hours_elapsed >= sla_hours[current_stage]:
			...
			frappe.db.set_value("BBF Material Inspection", insp.name, {
	...
	for user in recipients:
		if frappe.db.exists("User", user):
			frappe.get_doc({
				...
				"for_user": user,
				"type": "Alert",
				"document_type": "BBF Material Inspection",
...
def _send_inspection_notification_raw(recipients, subject, message, insp_name):
	...
	link = frappe.utils.get_url_to_form("BBF Material Inspection", insp_name)
	html_message = f"""
	<p>{message}</p>
