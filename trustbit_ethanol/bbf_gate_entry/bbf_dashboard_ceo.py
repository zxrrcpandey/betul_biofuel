...
def _get_kpis(today_date, start_date, end_date, company, fiscal_year, fy_start, fy_end):
	...
	pending_pos = frappe.db.sql("""
		...
		WHERE docstatus = 0
		AND (bbf_approval_status LIKE 'Pending CEO%%'
			OR bbf_approval_status LIKE 'Pending MD%%'
...
def _get_action_items():
	...
	pos = frappe.db.sql("""
		SELECT name, supplier_name, grand_total, bbf_purchase_category as category,
			bbf_approval_status as status, bbf_current_step as step,
			bbf_total_steps as total_steps, modified
		FROM `tabPurchase Order`
		WHERE docstatus = 0
		AND (bbf_approval_status LIKE 'Pending CEO%%'
			OR bbf_approval_status LIKE 'Pending MD%%'
	...
	mrs = frappe.db.sql("""
		SELECT name, cost_center, bbf_mr_status as status,
			bbf_mr_current_step as step, bbf_mr_total_steps as total_steps, modified
...
def _get_daily_ops(today_date):
	...
	week_row = frappe.db.sql("""
		SELECT COUNT(*) as entries,
			SUM(CASE WHEN status = 'Exited' THEN 1 ELSE 0 END) as exits
		FROM `tabBBF Token` WHERE entry_date BETWEEN %s AND %s
	...
	weight_by_type = frappe.db.sql("""
		SELECT ge.material_flow,
			...
			COUNT(*) as cnt
		FROM `tabBBF Weighbridge Log` wl
		JOIN `tabBBF Gate Entry` ge ON wl.gate_entry = ge.name
		JOIN `tabBBF Token` t ON wl.token_number = t.name
		WHERE t.entry_date = %s AND wl.net_weight > 0
		AND (wl.stock_direction IS NULL OR wl.stock_direction = 'Stock IN' OR wl.stock_direction = '')
...
def _get_budget_overview(fiscal_year, company, fy_start, fy_end):
	...
	overrides = frappe.db.sql("""
		...
		FROM `tabBBF Budget Override Log`
