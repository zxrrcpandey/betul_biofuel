...
def download_template():
	...
	return {
		...
		"filename": "bbf_bulk_item_template.csv"
	}
			...
			if not variant_codes_str:
				...
				for vc in variant_codes:
					if variant_source == "Brand":
						...
						if not frappe.db.exists("BBF Variant", vc):
							errors.append(f"BBF Variant '{vc}' not found")
...
def bulk_create_items(rows):
	"""Create items in bulk. Each row creates a BBF Item Creator doc + calls create_item().
...
def _create_single_item(row):
	"""Create a single item from a row dict via BBF Item Creator."""
	has_variant = (row.get("has_variant", "") or "").lower() in ("yes", "1", "true")
	variant_source = row.get("variant_source", "") or ""
	variant_codes_str = row.get("variant_codes", "") or ""
	maintain_stock = (row.get("maintain_stock", "") or "yes").lower() in ("yes", "1", "true")

	# Build BBF Item Creator doc
	doc_data = {
	...
	# Insert BBF Item Creator doc (triggers before_save → serial assignment + code building)
