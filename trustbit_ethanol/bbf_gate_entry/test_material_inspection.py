...
def execute():
	...
	def s1():
		...
		insp = frappe.get_all("BBF Material Inspection", filters={"gate_entry": ge.name}, limit=1)
		assert len(insp) == 1, f"Expected 1 inspection, got {len(insp)}"
		insp_doc = frappe.get_doc("BBF Material Inspection", insp[0].name)
	...
	def s2():
		...
		insp = frappe.get_all("BBF Material Inspection", filters={"gate_entry": ge.name})
	...
	def s3():
		_enable_inspection()
		token, ge = _create_non_rm_gate_entry()
		insp = frappe.get_doc("BBF Material Inspection", {"gate_entry": ge.name})
		insp.approve_all()
		insp.reload()
