...
def submit_budget_proposal(proposal_name):
    ...
    doc = frappe.get_doc("BBF Budget Proposal", proposal_name)
    ...
    # Fetch last year reference data
    _populate_last_year_data(doc)
...
def approve_budget_proposal(proposal_name, comment=""):
    ...
    if not frappe.has_permission("BBF Budget Proposal", "write"):
        frappe.throw(_("You don't have permission to approve budget proposals"))
...
def fetch_last_year_data(cost_center, fiscal_year):
    ...
    prev_fy_end = add_months(getdate(fy.year_end_date), -12)
    ...
    gl_entries = frappe.db.sql("""
        ...
        WHERE cost_center = %s
            ...
            AND is_cancelled = 0
        GROUP BY account
    """, (cost_center, prev_fy), as_dict=True)
    ...
    for acc in expense_accounts:
        ...
        if budget_amt or actual_amt:  # Only include accounts with data
            accounts.append({
                "account": acc.name,
...
def check_budget_for_po(docname):
    ...
    if not fiscal_year:
        fiscal_year = frappe.db.get_value("Fiscal Year", {"disabled": 0}, order_by="year_start_date desc")

    po_amount = flt(doc.grand_total)

    # Get budget for this CC + FY
    budget_info = _get_budget_info(cost_center, fiscal_year, company)
    ...
    # Check monthly budget if applicable
    monthly_info = _get_monthly_budget_info(cost_center, fiscal_year, company)
    ...
    if monthly_info:
        result["monthly_budget"] = monthly_info["monthly_budget"]
        result["monthly_committed"] = monthly_info["monthly_committed"]
        result["monthly_actual"] = monthly_info["monthly_actual"]
        result["monthly_available"] = monthly_info["monthly_available"]
...
def validate_budget_on_po_submit(doc):
    ...
    Returns budget_status dict. Throws if exceeded (unless CEO override pending).
    """
    cost_center = doc.cost_center if hasattr(doc, "cost_center") and doc.cost_center else None
    if not cost_center:
        for item in (doc.get("items") or []):
    ...
    if not budget_info["has_budget"]:
        # No budget configured — allow PO (budget is optional per CC)
...
def ceo_budget_override(docname, reason):
    ...
    # Log the override
    log = frappe.get_doc({
        "doctype": "BBF Budget Override Log",
        "parent": doc.name,
        "parenttype": "Purchase Order",
        "parentfield": "bbf_budget_override_log",
    ...
    doc.db_set("bbf_budget_overridden", 1, update_modified=True)
...
def _get_budget_info(cost_center, fiscal_year, company):
...
def _get_monthly_budget_info(cost_center, fiscal_year, company):
    ...
    # Get current month's accumulated budget percentage
    ...
    # Get accumulated percentage up to current month
    accumulated_pct = 0
    ...
    for row in dist_doc.percentages:
        ...
        # Check if this month is within the elapsed period
        # For Indian FY (Apr-Mar), we need to check from April
        if fy_start.month <= month_num <= 12 and month_num <= current_month:
            accumulated_pct += flt(row.percentage_allocation)
        elif month_num < fy_start.month and month_num <= current_month:
            accumulated_pct += flt(row.percentage_allocation)
    ...
    # Get monthly committed + actual (same queries but for comparison)
    budget_info = _get_budget_info(cost_center, fiscal_year, company)
...
def _create_erpnext_budget(proposal):
    """Create an ERPNext Budget record from an approved BBF Budget Proposal."""
    # Check if budget already exists for this CC + FY
    ...
    if existing:
        ...
        budget = frappe.get_doc("Budget", existing[0].name)
...
def get_budget_dashboard(fiscal_year=None, company=None):
    ...
    # Get committed (submitted POs) per CC
    committed_data = {}
...
def _send_budget_notification(doc, action, recipients, extra=None):
    ...
    extra = extra or {}
    subjects = {
        "pending": f"[Action Required] Budget Proposal for {doc.cost_center} ({doc.fiscal_year}) awaiting approval",
        "approved": f"[Approved] Budget for {doc.cost_center} ({doc.fiscal_year}) — Approved & Activated",
        "revised": f"[Revision Required] Budget Proposal for {doc.cost_center} sent back",
        "rejected": f"[Rejected] Budget Proposal for {doc.cost_center} ({doc.fiscal_year})",
    }
    subject = subjects.get(action, f"Budget Proposal {doc.name} — Update")
    ...
    for user in recipients:
        try:
            ...
            notification.document_type = "BBF Budget Proposal"
            ...
            frappe.log_error(
                title=f"Budget Notification Error ({doc.name} → {user})",
                message=frappe.get_traceback())

    try:
        amount_str = frappe.format_value(flt(doc.total_proposed), {"fieldtype": "Currency"})
        site_url = frappe.utils.get_url()
        doc_url = f"{site_url}/app/bbf-budget-proposal/{doc.name}"
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px;">
            ...
            <table style="border-collapse: collapse; width: 100%; margin: 10px 0;">
                <tr><td style="padding: 5px; font-weight: bold;">Cost Center:</td><td style="padding: 5px;">{frappe.utils.escape_html(doc.cost_center)}</td></tr>
                <tr><td style="padding: 5px; font-weight: bold;">Fiscal Year:</td><td style="padding: 5px;">{frappe.utils.escape_html(doc.fiscal_year)}</td></tr>
                <tr><td style="padding: 5px; font-weight: bold;">Proposed Total:</td><td style="padding: 5px;">{amount_str}</td></tr>
                <tr><td style="padding: 5px; font-weight: bold;">Status:</td><td style="padding: 5px;">{frappe.utils.escape_html(doc.status)}</td></tr>
            </table>
            {reason_html}
            <p><a href="{doc_url}" style="background: #2490EF; color: white; padding: 8px 16px; text-decoration: none; border-radius: 4px;">View Proposal</a></p>
        </div>
        """
        ...
        frappe.sendmail(
            ...
            message=message,
            now=True,
            reference_doctype="BBF Budget Proposal",
            reference_name=doc.name,
        )
    except Exception:
        frappe.log_error(
            title=f"Budget Email Error ({doc.name})",
            message=frappe.get_traceback())
