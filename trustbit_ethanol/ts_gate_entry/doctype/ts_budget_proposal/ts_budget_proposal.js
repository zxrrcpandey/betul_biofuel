frappe.ui.form.on("TS Budget Proposal", {
	refresh(frm) {
	...
	if (status === "Draft" && !frm.is_new()) {
		frm.add_custom_button(__("Submit to CEO"), () => {
			frappe.confirm(
				...
				() => {
					frappe.call({
						method: "trustbit_ethanol.ts_gate_entry.ts_budget.submit_budget_proposal",
		...
		frm.add_custom_button(__("Approve Budget"), () => {
			const d = new frappe.ui.Dialog({
				...
				primary_action(values) {
					d.hide();
					frappe.call({
						method: "trustbit_ethanol.ts_gate_entry.ts_budget.approve_budget_proposal",
		...
		frm.add_custom_button(__("Revise"), () => {
			const d = new frappe.ui.Dialog({
				...
				primary_action(values) {
					d.hide();
					frappe.call({
						method: "trustbit_ethanol.ts_gate_entry.ts_budget.revise_budget_proposal",
		...
		frm.add_custom_button(__("Reject"), () => {
			const d = new frappe.ui.Dialog({
				...
				primary_action(values) {
					d.hide();
					frappe.call({
						method: "trustbit_ethanol.ts_gate_entry.ts_budget.reject_budget_proposal",
						args: { proposal_name: frm.doc.name, reason: values.reason },
						freeze: true,
						callback() { frm.reload_doc(); },
						error() { frm.reload_doc(); }
	...
	if (status === "Revised" && is_proposer) {
		frm.add_custom_button(__("Resubmit to CEO"), () => {
			frappe.confirm(
				...
				() => {
					frappe.call({
						method: "trustbit_ethanol.ts_gate_entry.ts_budget.resubmit_budget_proposal",
						args: { proposal_name: frm.doc.name },
						freeze: true,
						callback() { frm.reload_doc(); },
						error() { frm.reload_doc(); }
...
function _fetch_last_year(frm) {
	frappe.call({
		method: "trustbit_ethanol.ts_gate_entry.ts_budget.fetch_last_year_data",
		args: {
