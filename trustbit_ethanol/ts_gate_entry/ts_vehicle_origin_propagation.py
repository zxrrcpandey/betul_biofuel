"""v2.9.14 — Vehicle Origin propagation.

G1 (TS Gate Entry) is the source of truth. Two propagation paths:
1. G1 → G2 (TS Token): on TS Gate Entry on_update, copy vehicle_origin to the
   linked TS Token (referenced via gate_entry.token_number) IF the Token's
   vehicle_origin is empty (don't overwrite operator's override).
...
def propagate_to_token(doc, method=None):
    if not doc.get("vehicle_origin") or not doc.get("token_number"):
        return
    token_origin = frappe.db.get_value("TS Token", doc.token_number, "vehicle_origin")
    if not token_origin:
