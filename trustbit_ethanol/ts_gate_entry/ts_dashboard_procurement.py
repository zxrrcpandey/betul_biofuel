...
def get_procurement_dashboard(date_range="This Month", company=None, category=None, start=None, end=None):
	...
	cat_filter = f"AND po.ts_purchase_category = {frappe.db.escape(category)}" if category else ""
	...
	rows = frappe.db.sql(f"""
		SELECT
			...
			CASE
				...
				WHEN po.ts_approval_status = 'Revised' THEN 'Revised'
...
def _get_pending_steps(filters, cat_filter):
	...
	rows = frappe.db.sql(f"""
		SELECT
			po.ts_approval_status as status,
			po.ts_current_step as current_step,
			po.ts_total_steps as total_steps,
			COUNT(*) as cnt,
...
def _get_avg_cycle_time(start_date, end_date, filters, cat_filter):
	...
	rows = frappe.db.sql(f"""
		SELECT
			IFNULL(po.ts_purchase_category, 'Uncategorized') as category,
			COUNT(*) as cnt,
			AVG(TIMESTAMPDIFF(HOUR, po.creation, po.ts_approved_date)) as avg_hours
		FROM `tabPurchase Order` po
		WHERE po.docstatus = 1
		AND po.ts_approved_date IS NOT NULL
	...
	overall = frappe.db.sql(f"""
		SELECT AVG(TIMESTAMPDIFF(HOUR, po.creation, po.ts_approved_date)) as avg_hours
		FROM `tabPurchase Order` po
		WHERE po.docstatus = 1
		AND po.ts_approved_date IS NOT NULL
		AND po.ts_approved_date BETWEEN %s AND %s
...
def _get_mr_pipeline(start_date, end_date, filters):
	...
	rows = frappe.db.sql(f"""
		SELECT
			CASE
				WHEN mr.ts_mr_status IN ('', 'Draft') OR mr.ts_mr_status IS NULL THEN 'Draft'
				WHEN mr.ts_mr_status LIKE 'Pending%%' THEN 'Pending'
				WHEN mr.ts_mr_status LIKE 'On Hold%%' THEN 'On Hold'
				WHEN mr.ts_mr_status = 'Approved' THEN 'Approved'
				WHEN mr.ts_mr_status = 'Rejected' THEN 'Rejected'
				WHEN mr.ts_mr_status = 'Revised' THEN 'Revised'
				ELSE mr.ts_mr_status
			END as status_group,
			COUNT(*) as cnt
...
def _get_summary(start_date, end_date, filters, cat_filter):
	...
	totals = frappe.db.sql(f"""
		SELECT
			...
			SUM(CASE WHEN po.docstatus = 0 AND po.ts_approval_status LIKE 'Pending%%' THEN 1 ELSE 0 END) as pending_count,
			SUM(CASE WHEN po.docstatus = 0 AND po.ts_approval_status = 'Rejected' THEN 1 ELSE 0 END) as rejected_count,
