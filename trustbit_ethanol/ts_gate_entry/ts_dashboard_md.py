...
def _get_md_pin():
	...
	try:
		return get_decrypted_password("TS Settings", "TS Settings", "md_dashboard_pin")
	except Exception:
		return None
...
def _get_company_overview(today_date, start_date, end_date, fiscal_year, fy_start, fy_end):
	...
	for company in companies:
		...
		pending_pos = frappe.db.sql(f"""
			...
			AND ts_approval_status LIKE 'Pending%%'
		""")[0][0] or 0
	...
	total_pending = frappe.db.sql("""
		SELECT COUNT(*) FROM `tabPurchase Order`
		WHERE docstatus = 0
		AND (ts_approval_status LIKE 'Pending MD%%'
...
def _get_action_items():
	...
	pos = frappe.db.sql("""
		SELECT name, supplier_name, grand_total, company,
			ts_purchase_category as category,
			ts_approval_status as status, modified
		FROM `tabPurchase Order`
		WHERE docstatus = 0
		AND (ts_approval_status LIKE 'Pending MD%%'
			OR ts_approval_status LIKE 'Awaiting%%Send%%MD%%')
...
def _get_procurement(start_date, end_date):
	...
	pipeline = frappe.db.sql("""
		SELECT
			IFNULL(ts_purchase_category, 'Uncategorized') as category,
			company,
			CASE
				WHEN ts_approval_status IN ('', 'Draft') OR ts_approval_status IS NULL THEN 'Draft'
				WHEN ts_approval_status LIKE 'Pending%%' THEN 'Pending'
				WHEN ts_approval_status LIKE 'Awaiting%%' THEN 'Awaiting'
				WHEN ts_approval_status = 'Approved' THEN 'Approved'
				WHEN ts_approval_status = 'Rejected' THEN 'Rejected'
				ELSE ts_approval_status
			END as status_group,
			COUNT(*) as cnt,
