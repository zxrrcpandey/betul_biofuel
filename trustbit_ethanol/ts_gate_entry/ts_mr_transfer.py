...
When `material_request_type == "Material Transfer"`, MRs follow a separate
approval flow:
...
type-branch in mr_before_save does NOT fire and Transfer MRs route through
...
def _is_flow_enabled():
	...
	this module entirely and Transfer MRs use the legacy Purchase chain.
	"""
...
def handle_before_save(doc):
	...
	== "Material Transfer". Validates Transfer-specific invariants WITHOUT
	running any Purchase chain logic.

	Caller has already run `_block_gate_field_tampering` (Lesson 162) which
	stays applied for Transfer too.
	"""
...
def _validate_transfer_for_submit(doc):
	...
	if not doc.items or len(doc.items) == 0:
		frappe.throw(_("Material Transfer must have at least one item."))
	...
	for idx, row in enumerate(doc.items, start=1):
		...
		if not row.get("from_warehouse"):
			frappe.throw(_("Row {0}: From Warehouse is required.").format(idx))
		if not row.get("warehouse"):
			frappe.throw(_("Row {0}: Target Warehouse is required.").format(idx))
		...
		if row.from_warehouse == row.warehouse:
			frappe.throw(
				_("Row {0}: From Warehouse and Target Warehouse must differ.").format(idx)
			)
...
def submit_for_stores_approval(mr_name):
	...
	if doc.material_request_type != "Material Transfer":
		frappe.throw(_("This endpoint is only available for Material Transfer MRs."))
...
def approve_transfer(mr_name):
	...
	try:
		...
		# Force Material Transfer purpose (mapper uses MR's type, but be explicit)
		se.purpose = "Material Transfer"
		se.stock_entry_type = "Material Transfer"
		# Copy serial_no from MR Item rows (match by item_code + idx)
		for s_idx, se_row in enumerate(se.items):
			# Best-effort match by item_code + idx position
...
def revert_on_stock_entry_cancel(doc, method=None):
	...
	Material Transfer MR, revert the MR back to Pending Stores Manager so
	it can be re-approved (or rejected).
...
def _compute_transfer_actions(status, is_creator, is_stores_manager, is_admin, docstatus):
	...
	if status == "Not Submitted" and (is_creator or is_admin):
		actions.append({
			...
			"primary": True,
			"icon": "📤",
			"confirm_msg": _("Submit this Material Transfer to the Stores Manager for approval?"),
		})
