frappe.ui.form.on("TS Return Item Transaction", {
	refresh(frm) {
		// Status indicator
		const status_map = {
			"Draft": "orange",
		...
		if (frm.doc.status === "Draft" && frm.doc.transaction_type === "Discard") {
			frm.add_custom_button(__("Submit for Approval"), () => {
				frappe.confirm(
					...
					() => {
						frappe.call({
							method: "trustbit_ethanol.ts_return_item_tracker.ts_return_item_api.submit_for_discard_approval",
		...
		if (frm.doc.status === "Draft" && frm.doc.transaction_type !== "Discard") {
			frm.add_custom_button(__("Complete Transaction"), () => {
				frappe.confirm(
					...
					() => {
						frappe.call({
							method: "trustbit_ethanol.ts_return_item_tracker.ts_return_item_api.complete_transaction",
							args: { transaction_name: frm.doc.name },
							callback: () => {
		...
		if (frm.doc.status === "Pending Approval") {
			frm.add_custom_button(__("Approve Discard"), () => {
				frappe.call({
					method: "trustbit_ethanol.ts_return_item_tracker.ts_return_item_api.approve_discard",
					args: { transaction_name: frm.doc.name },
					callback: () => frm.reload_doc(),
				});
			...
			frm.add_custom_button(__("Reject Discard"), () => {
				frappe.prompt(
					...
					(values) => {
						frappe.call({
							method: "trustbit_ethanol.ts_return_item_tracker.ts_return_item_api.reject_discard",
